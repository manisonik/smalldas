    ...
    public sealed class DynamicTypeDescriptor : ICustomTypeDescriptor, INotifyPropertyChanged
    {
        ...
        public sealed class DynamicProperty : PropertyDescriptor, INotifyPropertyChanged
        {
                ...
                : base(name, attrs)
            {
                ...
                DefaultValueAttribute def = GetAttribute<DefaultValueAttribute>(Attributes);
                if (def == null)
                {
            ...
            public override bool IsReadOnly
            {
                get
                {
                    ...
                    ReadOnlyAttribute att = GetAttribute<ReadOnlyAttribute>(Attributes);
                    if (att == null)
                        return false;
        ...
        public void SwapProperty(string propertySrc, string propertyDest)
        {
            if (propertySrc == null)
                throw new ArgumentNullException("propertySrc");

            if (propertyDest == null)
                throw new ArgumentNullException("propertyDest");

            int idx = -1;
            for (int i = 0; i < Properties.Count; i++)
            {
                if (Properties[i].Name == propertySrc)
                {
                    idx = i;
                }
            }

            PropertyDescriptor dest = null;
            foreach (PropertyDescriptor pd in Properties)
            {
                if (pd.Name == propertyDest) {
                    dest = pd;
                    break;
                }
            }
